    ...
    <div className="collection">
      ...
      <div className={`relative h-screen w-full mb-10  ${!collection.image ? 'bg-gray-300' : ''}`} style={{ maxHeight: '300px' }} >
        ...
        <div class="absolute inset-0 mt-8 flex w-full flex-col items-center justify-center p-5 text-center md:px-20 lg:space-y-10">
          <h1 class="text-2xl font-bold tracking-tight text-heading lg:text-4xl xl:text-5xl">{collection.title}</h1>
      ...
      <Pagination connection={collection.products}>
        {({ nodes, isLoading, PreviousLink, NextLink }) => (
          <>
            ...
            <br />
            <NextLink>
              {isLoading ? <button className='flex items-center rounded-md bg-slate-900 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-300 m-auto'>Loading... </button> : <button className='flex items-center rounded-md bg-slate-900 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-300 m-auto'>Load more ↓</button>}
            </NextLink>
          </>
        )}
      </Pagination>
...
function ProductItem({ product, loading }) {
  ...
  return (
    <Link
      className="product-item relative rounded-lg bg-white shadow-md overflow-hidden"
      key={product.id}
      prefetch="intent"
      to={variantUrl}
    >
      ...
      <div className='content mt-4 px-5 pb-5'>
        ...
        <div class="flex items-center justify-between">
          <p>
            <span class="text-3xl font-bold text-slate-900"><Money data={product.priceRange.minVariantPrice} /></span>
            <span class="text-sm text-slate-900 line-through"> $249 </span>
          </p>
